def maxminest (record, dur_ratio = 1,CFD_qnt = [0.98] ):
    ...
    if max(CFD_qnt) >= 1 or min(CFD_qnt) <=0:
        raise ValueError('CFD value must be greater than zero and less than one')


    CFD_qnt = np.array(CFD_qnt)
    ...
    max_qnt = np.zeros((rec_size,1))
    min_qnt = np.zeros((rec_size,1))
    ...
    for i in np.arange(rec_size):
        ...
        y_pk = np.sqrt(2.0*np.log(np.divide(-dur_ratio*Nupcross,np.log(CFD_qnt))))
